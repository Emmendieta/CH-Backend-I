...
ROUTER.post("/", async (req, res) => {
    try {
        ...
        if (!title || !description || !code || !price || !status || !stock || !category || !thumbnails) { return res.status(400).json({ error: `Error: Todos los campos son obligatorios para poder proceder a dar el alta del producto!` }); }
...
ROUTER.put("/:pid", async (req, res) => {
    try {
        ...
        let verify = await PRODUCT_MANAGER.updateProduct(pid, title, description, code, price, status, stock, category, thumbnails);
